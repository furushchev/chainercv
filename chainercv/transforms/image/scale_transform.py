...
def scale(x, ratio=None, max_shape=None, min_shape=None):
    """Scale image to match the given shape.

    Args:
        x (~numpy.ndarray): array to be transformed. This is in CHW format.
        ratio (float): scale ratio.
        max_shape (tuple): this is a tuple of length 2. Its elements are
            ordered as (height, width).
        min_shape (tuple): this is a tuple of length 2. Its elements are
            ordered as (height, width).
    ...
    if max_shape is not None and len(max_shape) != 2:
        raise ValueError('length of the max_shape needs to be 2')
    if min_shape is not None and len(min_shape) != 2:
        raise ValueError('length of the min_shape needs to be 2')
    x = x.transpose(1, 2, 0)
